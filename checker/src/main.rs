...
struct Args {
    /// Bool options are for whether flags exist (-h, --compile etc.)
    help: bool,
    /// Stores something if the user specified it (--age johnsmith)
    age_username: Option<String>,
    country_username: Option<String>,
    compile: bool,
    compile_onefile: bool,
    /// Any arguments that were not recognised in the struct
    free: Vec<String>,
}
...
OPTIONS:
        --age <username>        Prints age interpolation debug info for the given username
        --country <username>    Prints country interpolation debug info for the given username
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Parse the arguments into an Args struct
    let args = Args {
        help: args.contains(["-h", "--help"]),
        age_username: args.opt_value_from_str("--age")?,
        country_username: args.opt_value_from_str("--country")?,
        compile: args.contains(["-c", "--compile"]),
        compile_onefile: args.contains(["-1", "--compile-onefile"]),
        free: args.free()?,
    };

    // Check whether we should display the help message
    if args.help {
    ...
    let debug_age_username = args.age_username;
    let debug_country_username = args.country_username;
    let is_debugging: bool =
        debug_age_username.is_some() || debug_country_username.is_some();

    // Assume there is one free argument or none
    let search_root =
        if args.free.is_empty() { meet_data_root.clone() }
        else {
