...
fn bd_range_subset_is_consistent(
    subset_vec: &[usize],
...
// Finds the distance of a point from a vector of birthdate ranges and returns
...
// Check a vector of sample points to find the largest LCS possible (if any) at
// one of these points
fn find_lcs_numeric(
    subset_vec: &[usize],
    bd_range_vec: &[BirthDateRange],
...
fn get_test_points(subset_vec: &[usize], bd_range_vec: &[BirthDateRange]) -> Vec<f32> {
    ...
    test_points.push(
        (sorted_ranges[0].1 + ((sorted_ranges[0].1 + sorted_ranges[1].0) / 2.0)) / 2.0,
    );
...
// Version of the LCS algorithm that uses where the error curves intersect to
// potentially reduce the numer of calls
fn find_lcs_algebraic(
    subset_vec: &[usize],
    bd_range_vec: &[BirthDateRange],
) -> Option<Vec<usize>> {
    let mut best_errors = Vec::new();
...
fn find_lcs(subset_vec: &[usize], bd_range_vec: &[BirthDateRange]) -> Option<Vec<usize>> {
    ...
    let numeric_ops = (test_vals.len() as f32)
        * (subset_vec.len() as f32)
        * (subset_vec.len() as f32).log(2.0);
...
// Group data by consistent subsets of age data, returns the indices of elements
...
pub fn group_by_age(
    ...
    // Add this group to our list of groups and find the LCS of the remaining
    // elements
    while lcs.is_some() {
...
pub fn print_groups(
    ...
    for (ii, group) in bd_groups.iter().enumerate() {
        ...
        for range_idx in group {
            println!(
                "({},{}): {} ",
                bd_range_vec[*range_idx].min,
                bd_range_vec[*range_idx].max,
                date_vec[*range_idx]
            );
        }
    }
